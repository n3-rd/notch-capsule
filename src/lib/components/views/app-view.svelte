<script lang="ts">
  ...
  let progressPercent = $derived(
    currentMedia && currentMedia.duration > 0
      ? (predictedElapsed / currentMedia.duration) * 100
      : 0
  );
  ...
  function updatePredictedTime() {
    ...
    if (currentMedia.is_playing) {
      ...
      predictedElapsed = Math.min(
        currentMedia.elapsed + timeSinceLastFetch,
...
{#if currentMedia}
  <div class="w-full h-full flex items-center px-4 py-3 overflow-hidden">
    <div class="flex gap-4 items-center w-full max-w-full">
      <!-- Album Art -->
      <div class="w-24 h-24 rounded-2xl overflow-hidden shrink-0 shadow-lg bg-white/5">
        {#if artworkUrl}
          <img 
            src={artworkUrl} 
          ...
          />
        {:else}
          <div class="w-full h-full flex items-center justify-center text-white/30 text-2xl">
            🎵
          </div>
        {/if}
      </div>
      ...
      <div class="flex flex-col gap-2 min-w-0 flex-1">
        ...
        <div class="flex flex-col gap-0.5">
          <h2 class="text-xl font-bold text-white leading-tight truncate">{currentMedia.title}</h2>
        ...
        <div class="flex items-center gap-2">
          <span class="text-xs text-white/50 shrink-0 tabular-nums">{formatTime(predictedElapsed)}</span>
          <div 
            class="w-full h-1.5 bg-white/10 rounded-full overflow-hidden cursor-pointer group"
            onclick={handleSeek}
            onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleSeek(e as unknown as MouseEvent); }}
            role="slider"
            tabindex="0"
          ...
          >
            <div 
              class="h-full bg-white rounded-full transition-all duration-100 ease-linear group-hover:bg-white/90"
              style="width: {Math.min(progressPercent, 100)}%"
            ></div>
          </div>
          <span class="text-xs text-white/50 shrink-0 tabular-nums">{formatTime(currentMedia.duration)}</span>
        </div>
...
{:else}
  <div class="w-full h-full flex items-center justify-center px-4 py-3">
    <div class="text-center text-white/50">
      <div class="text-3xl mb-2">🎵</div>
      <p class="text-sm">No media playing</p>
      <p class="text-xs mt-1">Play something in any app</p>
    </div>
  </div>
{/if}
